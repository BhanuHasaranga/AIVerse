    ...
    if not data.empty and len(data) >= 3:
        ...
        with st.expander("📐 Step-by-Step Calculation", expanded=True):
            ...
            if correlation > 0.7:
                ...
                st.write(f"⚠️ **Moderate positive relationship** (r = {correlation:.2f})")
            elif correlation > -0.3:
                st.write(f"❌ **Weak/No relationship** (r = {correlation:.2f})")
            elif correlation > -0.7:
                st.write(f"⚠️ **Moderate negative relationship** (r = {correlation:.2f})")
            else:
...
with col2:
    ...
    def ml_usage():
        ...
        st.write("**1️⃣ Feature Selection**")
        st.write("High correlation = redundant features")
        
        st.write("**2️⃣ Dimensionality Reduction**")
        st.write("PCA uses covariance matrix")
        
        st.write("**3️⃣ Anomaly Detection**")
